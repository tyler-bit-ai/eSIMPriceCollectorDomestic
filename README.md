...
- `시점별 변경 상품`: 선택한 2개 시점 사이에서 가격 또는 노출 여부가 달라진 상품만 별도 확인
- `상세 비교표`: 대표 옵션, 최저가, 옵션 수, 원본 링크 확인 및 CSV 다운로드
- `크롤링 시점 선택`: 드롭다운에서 원하는 시점 2개까지 체크해 시점별 가격 변화를 비교

대시보드 사용 포인트:

- 상단 드롭다운에서 publish 시점을 최대 2개까지 체크할 수 있고, 먼저 체크한 시점이 기준이 됩니다.
- `시점별 변경 상품` 섹션에는 두 시점 사이에서 가격이 달라졌거나 한쪽 시점에만 존재하는 상품만 표시됩니다.
...
1. 스냅샷 2개 이상이 있는 상태에서 드롭다운에서 시점 2개를 체크하고, 먼저 체크한 시점이 기준으로 반영되는지 확인한다.
2. `시점별 변경 상품` 섹션이 `망 유형 가격 차이` 아래에 표시되고, 값이 달라진 상품만 노출되는지 확인한다.
3. `국가별 가격 분포 밴드` 카드 클릭 후 섹션의 `드릴다운 해제` 버튼으로 바로 원복되는지 확인한다.
4. `망 유형 가격 차이` 카드 클릭 후 동일하게 섹션 내에서 원복되는지 확인한다.
5. Excel 다운로드 CSV에 `시점비교` 열이 제거되고 현재 기준 시점 데이터만 내려가는지 확인한다.
